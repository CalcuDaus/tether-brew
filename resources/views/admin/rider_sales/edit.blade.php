        ...
        <form action="{{ route('admin.rider_sales.update', $riderSale->id) }}" method="POST" id="sales-form">
            ...
            <div style="display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 30px;">
                ...
                <div class="form-group" style="flex: 1; min-width: 250px;">
                    ...
                    <input type="hidden" name="rider_id" value="{{ $riderSale->rider_id }}">
                    <select class="form-input" disabled style="height: 48px; font-size: 1rem; background: rgba(0,0,0,0.05); cursor: not-allowed; color: var(--text-muted);">
            ...
            <div style="margin-bottom: 30px; padding: 25px; background: rgba(0,0,0,0.02); border: 1px solid var(--border-color); border-radius: 12px;">
                ...
                <div style="display: flex; gap: 20px; margin-bottom: 15px;">
                    <div class="form-group" style="flex: 1; margin: 0;">
                        ...
                        <input type="number" name="cash_amount" id="cash-amount" class="form-input" value="{{ $riderSale->cash_amount }}" {{ $finReadonly }} style="height: 44px; font-size: 1rem; font-weight: 600; {{ $finStyle }}">
                    </div>
                    <div class="form-group" style="flex: 1; margin: 0;">
                        <label class="form-label" style="font-weight: 600;">QRIS</label>
                        <input type="number" name="qris_amount" class="form-input" value="{{ $riderSale->qris_amount }}" {{ $finReadonly }} style="height: 44px; font-size: 1rem; font-weight: 600; {{ $finStyle }}">
                    </div>
                </div>
                ...
                <div style="display: flex; gap: 20px; margin-bottom: 15px;">
                    <div class="form-group" style="flex: 1; margin: 0;">
                        ...
                        <input type="number" name="actual_setor" id="actual-setor" class="form-input" value="{{ $riderSale->actual_setor }}" {{ !$isBar ? 'required' : '' }} {{ $finReadonly }} style="height: 44px; font-size: 1rem; font-weight: 600; {{ !$isBar ? 'border-color: #3b82f6;' : '' }} {{ $finStyle }}">
                    </div>
                    <div class="form-group" style="flex: 1; margin: 0;">
                ...
                <div style="display: flex; gap: 20px; margin-bottom: 15px;">
                    ...
                    <div class="form-group" style="flex: 1; margin: 0;">
                        ...
                        <input type="number" id="total-cup" class="form-input" readonly value="{{ $riderSale->items->sum('stock_sold') }}"
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 20px;">
                <div class="form-group" style="flex: 1; max-width: 300px; margin: 0;">
                    ...
                    <input type="text" name="admin_pemeriksa" class="form-input" value="{{ $riderSale->admin_pemeriksa ?? auth()->user()->name }}" {{ $finReadonly }} style="height: 48px; font-size: 1rem; {{ $finStyle }}">
                </div>
                    ...
                    if (stockInfo) {
                        ...
                        if (totalInput > maxAvailable) {
                            Swal.fire({
                                ...
                                text: `Stok tersedia untuk ${stockInfo.product_name} adalah ${maxAvailable} Pcs. Anda menginput total ${totalInput} Pcs (Keluar: ${stockOut}, Tambahan: ${stockAdded}).`,
                                icon: 'error',
